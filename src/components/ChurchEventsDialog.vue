  ...
  >
    <q-card class="entity-dialog church-list-dialog">
      <header class="entity-dialog__header">
        <div>
          <h2 class="entity-dialog__title">Events participated by {{ churchName || "this church" }}</h2>
          <p class="entity-dialog__subtitle">{{ rows.length }} event(s)</p>
        </div>
      ...
      <q-card-section class="church-list-dialog__body">
        ...
        >
          <template #body-cell-eventDate="props">
            <q-td :props="props">
              <span class="church-list-dialog__muted">{{ formatDate(props.row.eventDate) }}</span>
            </q-td>
          </template>

          <template #body-cell-location="props">
            <q-td :props="props">
              <span class="church-list-dialog__muted">{{ props.row.location || "—" }}</span>
            </q-td>
          </template>
          ...
          <template #no-data>
            <div class="church-list-dialog__empty">
              <q-icon name="event_busy" size="20px" color="grey-5" />
              <span>No events found for this church.</span>
            </div>
          </template>
        </q-table>
      </q-card-section>
...
const columns = [
  ...
  { name: "participantCount", label: "Church participants", field: "participantCount", align: "right", sortable: true },
...
async function loadEvents() {
  ...
  loading.value = true;
  try {
    const { data } = await api.get(`/churches/${props.churchId}/events`);
    rows.value = data;
  } catch {
    $q.notify({ type: "negative", message: "Failed to load church events." });
    close();
  } finally {
    loading.value = false;
  }
